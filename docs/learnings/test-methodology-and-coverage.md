...
- **The same near-miss bites in reverse, from the workflow side.** `uncommented` is applied per *step block*, but the region-level greps in "node is set up before asar is installed" and "asar is installed by exactly one step" read the raw region — so writing `@electron/asar` into a YAML comment above the setup step reds three tests, because the comment counts as a second step that installs asar and as an asar reference preceding the Node setup. That is the invariant working (one install step, and it comes after Node), not a parser bug, so the fix belongs in the comment: spell shared tooling bare (`asar`, `the claude-code CLI`) in this region and leave the npm specifiers to the `run:` lines that actually install them. The `ci.yml` comment says so inline, since the next person to annotate that block will hit it.
