...
4. **Our `.deb` survives, renamed** (`claude-desktop-linux` + `Conflicts:/Replaces:/Provides: claude-desktop`, distinct install/AppArmor paths; AppStream ID frozen). The rename ships as its own defusal release (v2.1.0) before the acquisition swap; the branch keeps `claude-desktop` until then.
