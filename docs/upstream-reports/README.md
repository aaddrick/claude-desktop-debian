...
|---|---|---|---|
| Cowork virtiofsd probe: two hardcoded paths + Ubuntu-22-only bundled fallback ("requires QEMU" with a full KVM stack) | anthropics/claude-code | **drafted** — [`771-cowork-virtiofsd-probe.md`](771-cowork-virtiofsd-probe.md); reproduced on the official `.deb` (Pop!_OS datapoint in #771); downstream `virtiofsd-probe` asar patch + doctor fix ship meanwhile | [#771](https://github.com/aaddrick/claude-desktop-debian/issues/771) / [#772](https://github.com/aaddrick/claude-desktop-debian/issues/772) |
