...
- `tools/patch-necessity-audit.sh` runs again on every bundle upstream currently ships. It followed the single `require()` out of `index.js` and grepped whatever it resolved to — the 1.19367.0 shape, a stub plus exactly one content-hashed chunk — so on a 1.26832.0-or-later tree the `grep -oP` captured several filenames into one string and the `-f` test on it aborted the tool (`index.js requires index.chunk-aaa.js\nindex.chunk-bbb.js but it is missing`). The abort was the *safe* failure and the obvious fix is the dangerous one: taking the first chunk would leave every anchor living in another chunk reading zero, and a zero here renders as `not-needed`, which is indistinguishable from an anchor upstream genuinely fixed — a verdict that argues for deleting a patch still carrying load, since the matrix feeds [`docs/learnings/official-deb-rebase-verification.md`](docs/learnings/official-deb-rebase-verification.md). The tool now probes the whole `.vite/build` set instead of resolving one file, which sidesteps the resolution problem entirely because it only ever *counts* anchors and never edits one (the same "there is deliberately no main JS file any more" move `_resolve_anchor_file` made for the patch stage in [#820](https://github.com/aaddrick/claude-desktop-debian/pull/820)). Three latent bugs surfaced on the way: counting used `grep -c`, which answers "files with a match" and so reported 1 for a minified chunk carrying twelve occurrences on its single line; the file list was sorted in the operator's locale, where collation ignores punctuation and sorts `index2.chunk-*` *ahead* of `index.chunk-*`, making the tool's answers environment-dependent (now `LC_ALL=C`); and the autoupdater probe's BRE alternation `apt_channel_pending\|apt channel not yet live` would have become a *literal pipe* when moved to `grep -P`, silently flipping a `not-needed` patch to `check`. Running it on a current bundle for the first time then exposed a fourth: the autoupdater probe was still anchored on `apt_channel_pending`, which upstream renamed to `managed_by_package_manager` in the 1.18286.2 → 1.19367.0 window when the APT channel went live. The build's AU-1 tripwire moved with it; this probe had not, so it reported a false `check` against every bundle since — invisible because the tool aborted before reaching that row. Both names are now accepted, and the audit agrees with the tripwire again. The run section is wrapped in `main` behind a `${BASH_SOURCE[0]} == "$0"` guard so the probes can be unit-tested against fixture trees without fetching a 170 MB `.deb`. `--help` also prints its whole header again: the range was hardcoded as `sed -n '2,20p'`, correct when written and silently stale once the block grew to 21 lines, so the listing stopped after `needed?` and dropped the `check` verdict — the one most rows in the matrix actually carry. It now derives the extent from the closing banner. New `tests/patch-necessity-audit.bats` (25 tests) covers all three bundle shapes — single-file, stub-plus-one-chunk, and multi-chunk across two families — and fourteen mutations each turn a different test red, including the first-chunk "fix" itself, narrowing any of `count`/`has`/`extract` to one file, un-pinning the sort locale, reverting the updater anchor, and widening the preload probe off `mainView.js`. Verified end to end against the pinned official 1.52386.3 `.deb`, where the tool now probes 173 JS files and returns a full matrix with real evidence (`frame:!1 occurs 7x`, `TrayIconLinux refs: 2`, quick-window var `TW`, `coworkd refs: 4`) in place of an abort. ([#850](https://github.com/aaddrick/claude-desktop-debian/issues/850))
- `tools/patch-necessity-audit.sh` and `tests/test-artifact-common.sh` no longer reach `@electron/asar` through an unpinned `npx --yes @electron/asar`, the last two callers still carrying the trap [#841](https://github.com/aaddrick/claude-desktop-debian/pull/841) closed for the build. Both sit outside `setup_nodejs`'s reach, so the Node floor did not cover them: every `@electron/asar` 4.x release declares `engines.node >=22.12.0` and hard-refuses to start below it, npm reports that as an `EBADENGINE` **warning** rather than an error, and the wrapper installs, lands on disk, and fails on every invocation — on Debian 13 stable (Node 20.19.2) that is the default local experience, and whether an unpinned install even lands on 4.x depends on the npm version (npm 9 takes `latest`; npm 10's manifest picker skips engine-incompatible versions), so the symptom is host-dependent. In the audit tool the multi-word `asar_exec='npx --yes @electron/asar'` was also invoked unquoted, which turns `extract` into a package name. `tests/test-patch-stage.sh`'s `_resolve_asar` moves to `scripts/_common.sh` and all three now share it: prefer a real `asar` on `PATH`, else `npm install @electron/asar@<major>` into a caller-supplied directory, else fail loud — no `npx` path survives, and `"$asar_exec"` is quoted at every call site. Both new callers pin `@3` rather than `@4`: they only ever *read* an asar, which every major does identically, and 3.4.1 declares `engines.node >=10.12.0` so it runs anywhere. For the artifact tests that is load-bearing in CI too, not only on a laptop — `test-artifacts.yml` runs no `actions/setup-node` and takes whatever `apt-get install nodejs` (Ubuntu) or `dnf install nodejs` (the Fedora rpm-leg container) hands it, which is not guaranteed to clear 4.x's `>=22.12.0` floor. Pinning `@4` would buy nothing and would hard-fail both CI and every local run on the very host the bug was reported from. The build keeps resolving through `setup_asar` under the enforced `NODE_MIN_VERSION` floor, which is where the version actually matters because that is the path that *writes*. The shared resolver also inherits `setup_asar`'s two-arm run-check (exit code and stdout shape judged separately, merged streams reported, the match anchored at the start of the reply because the refusal text quotes the offending Node version). Separately, `validate_app_contents` reported `[PASS] Skipping asar extraction (tool not available)` when the tool was unusable, which silently dropped every assertion behind it — the `package.json` shape, `productName`, and the `StartupWMClass`/`desktopName` agreement that closes [#779](https://github.com/aaddrick/claude-desktop-debian/issues/779) — so an unresolvable asar or a failed extract now fails instead. Pinned by `tests/asar-resolve.bats`; fourteen mutations — dropping the pin, deleting either arm of the run-check, unanchoring the version match, dropping the `-x` and empty-argument guards, ignoring a `PATH` asar, dropping the Node floor from the failure message, reverting either call site to `npx`, unquoting `$asar_exec`, restoring the skip-`[PASS]`, capturing the resolver's output with `$(...)` instead of a redirect, and leaving a private copy of the resolver behind in the patch-stage harness — each turn a different test red. That `$(...)` case is not hypothetical: the first draft of the artifact-test call site had it, and because `_resolve_asar` publishes `$asar_exec` as a global, the subshell discarded the assignment and the extract invoked the empty string — which still fails, so both negative tests stayed green on the wrong cause until a positive test that has to reach a successful extract was added. ([#839](https://github.com/aaddrick/claude-desktop-debian/issues/839), [#846](https://github.com/aaddrick/claude-desktop-debian/pull/846))
- `--doctor` no longer reports `[PASS] SingletonLock: held by running process` for a stale lock whose PID has since been recycled by an unrelated process, and the launcher's stale-lock cleanup no longer keeps such a lock. Both decided "live" from `kill -0 $pid` alone, and PIDs are recycled, so any other readable same-user process inheriting the number made a dead instance's lock look held (a false green, not a launch blocker: Electron's own single-instance code connects to the `SingletonSocket` first and unlinks a lock whose holder does not answer, so the launcher half is hygiene that keeps the log honest). `kill -0` success is now additionally qualified by `_pid_is_claude_desktop`, which requires `claude-desktop` in the `/proc/PID/exe` path. It is keyed on the executable path rather than on the launcher's `--class` fingerprint or on an exe basename, because the lock is shared by every Claude Desktop build on the machine — the official Anthropic `.deb` (launched without `--class`), this project's deb/rpm/AppImage/Nix, and a pre-3.0 `electron/dist` tree still running across an upgrade — and each of those is a live holder that the doctor must not tell the user to `rm`; a basename set including bare `electron` would also count every unrelated `node_modules/.../electron` as live. A replaced binary's ` (deleted)` marker needs no special case, and an unreadable `/proc/PID/exe` (zombie, or a process that exec'd a setuid binary) counts as live, the harmless direction. The doctor downgrades the reused-PID case to the existing stale-lock `[WARN]` with an accurate parenthetical, the launcher removes the lock and logs that the PID was reused, and the cross-UID `EPERM` behaviour is unchanged. Mutation-checked in both directions, plus a case that turns red if the path test is ever narrowed to a basename match. ([#784](https://github.com/aaddrick/claude-desktop-debian/issues/784))
- The launcher now terminates a Claude Desktop UI whose executable was replaced underneath it by a package upgrade, so the next launch starts the newly installed build instead of silently doing nothing. When dpkg/rpm replaces the Electron binary while an instance is running, the old process keeps its deleted executable and Electron's single-instance lock; the relaunch then loses the lock to the stale process and exits without a window or an error. `cleanup_replaced_desktop_ui` matches a live UI on the `--class` fingerprint, requires the kernel's ` (deleted)` marker on `/proc/PID/exe` before touching anything, and escalates SIGTERM → SIGKILL only for those PIDs; a fingerprinted process whose binary is intact is never touched, pinned by a negative test. It is wired into all three launchers but only ever acts on deb and rpm installs — on AppImage the binary lives inside the FUSE mount, which stays valid after the `.AppImage` file is replaced, so the marker never appears and the call is a no-op by construction. Detection reads the marker with plain `readlink` rather than `readlink -f`, which fails when the install directory is gone too (a package migration or layout change, not just a same-layout file replace) and would silently miss the stale UI — pinned by a mutation-checked bats case that removes the whole install directory before the cleanup runs. Verified 121/121 on `launcher-common.bats` across Ubuntu 24.04/26.04, Debian 12/13, Fedora 42, openSUSE Leap 15.6, and Arch (bash 4.4→5.3, procps-ng 3.3.17→4.0.7).
- `tests/test-patch-stage.sh` no longer reports a phantom patch failure on machines without `asar` on `PATH`. Its fallback `asar_exec=$(command -v asar || command -v npx)` could never work: the harness invokes `"$asar_exec"` as a single word, so the fallback ran `npx extract-file app.asar package.json` — npx reads `extract-file` as a package name, fetches an unrelated `extract-file@1.0.0` from the registry, `package.json` is never extracted, and the WM_CLASS tripwire fails the build on an empty `desktopName` instead of anything real. `_resolve_asar` replaces it: prefer a real `asar`, else `npm install @electron/asar@3` into `$work_dir`, else fail loud — no `npx` path survives. The major is pinned because `@electron/asar` 4.x requires Node ≥ 22.12 and refuses to run on the Node 20 that Debian 13 ships, and whether an unpinned install lands on 4.x depends on the npm version (npm 9 takes `latest`, npm 10's manifest picker skips engine-incompatible versions) — so the resolver also probes `--version` rather than trusting `-x`, and stops with a message naming the Node requirement instead of letting a non-runnable binary fall through to the same phantom WM_CLASS failure ([#839](https://github.com/aaddrick/claude-desktop-debian/issues/839) tracks the build's own copy of this trap). Mutation-checked (restoring the old line re-selects `/usr/bin/npx`; dropping the `--version` probe lets a 4.x binary pass `-x`) and verified end-to-end against the pinned official `.deb` with `asar` absent from `PATH`.
- Local builds work again on hosts running Node.js older than v22.12, which is every default Debian 13 install (Node 20.19.2). `setup_nodejs` accepted any Node with a major of 20 or above and downloaded v20.18.1 when it had to supply one, but `@electron/asar` moved its floor to `engines.node >=22.12.0` at 4.0.0 and every 4.x release since hard-refuses to start below it. npm reports that as an `EBADENGINE` **warning** and installs the wrapper anyway, so the build got an asar that existed and never ran — and because the first thing the patch stage asks asar for is `package.json`, the failure surfaced as the `WM_CLASS` tripwire (`desktopName is missing/empty — cannot derive WM_CLASS`), which reads like upstream moved a field rather than like the toolchain is broken. The gate now compares major *and* minor against a `NODE_MIN_VERSION` of 22.12.0, the local fallback downloads v22.23.2 (the version CI's `ubuntu-latest` already builds on; its checksum is still resolved from the release's own `SHASUMS256.txt`, so nothing is pinned by hand), and `setup_asar` runs `asar --version` once before handing the executable on. That last check judges both the exit code and the shape of stdout, and reports the merged streams. 4.3.0 under Node 20.19.2 exits 1 with an empty stdout and its complaint on stderr, so the exit code catches the shape that ships today; the stdout arm is defense in depth for a future release that answers through `--version` and exits zero. Judging stdout rather than the merged stream matters either way, because the refusal text quotes the offending Node version — a "contains a version number" match on merged output would pass the very reply it exists to catch. `.claude/hooks/install-build-tools.sh` tracks the same floor. Pinned by `tests/dependencies.bats`; nine mutations — collapsing the comparison to the major, reverting either constant, dropping each arm of the run-check, merging the streams, unanchoring the version match, and dropping the unparseable-input guard — each turn a different test red. ([#839](https://github.com/aaddrick/claude-desktop-debian/issues/839))
- `tests/test-artifact-rpm.sh` now installs the rpm once with `rpm -ivh --nodeps --noscripts` and asserts the chrome-sandbox setuid bit on that install, before the existing scripted install runs. [#595](https://github.com/aaddrick/claude-desktop-debian/pull/595) moved the bit out of a `%post chmod` into the payload itself (a `chmod 4755` on the buildroot file in `%install`, recorded by the `%files` directory walk) so it survives installs that skip scriptlets, with `--noscripts` standing in for rpm-ostree/Silverblue layered images that CI cannot simulate; but the artifact test only ever installed with scriptlets enabled, so its assertion could not tell that fix apart from a regression back to a `%post chmod`. Mutation-checked: reverting the spec to a `%post chmod` turns only the new `--noscripts` assertion red while the scripted-install assertion stays green. ([#617](https://github.com/aaddrick/claude-desktop-debian/issues/617))
...
- `--doctor` no longer reports a false `[FAIL] Chrome sandbox: perms=755, owner=<you>` on AppImage runs, with a `sudo chown root:root /tmp/.mount_.../chrome-sandbox` fix hint that could never be followed. Confirmed on a locally built `1.49585.0` AppImage in a clean Debian 13 container as a non-root user, both via `--appimage-extract-and-run --doctor` and via `AppRun --doctor` on an extracted tree. Since [#816](https://github.com/aaddrick/claude-desktop-debian/pull/816) the check judges only the running binary's adjacent `chrome-sandbox`, which on AppImage is the copy inside the mount/extract directory: the AppDir permission normalization (`chmod u=rwX,go=rX`) drops the setuid bit and the extracted file is owned by the invoking user, so the `4755`/root test failed by construction, on a transient read-only path no `sudo` could repair — and moot regardless, because `build_electron_args` passes `--no-sandbox` unconditionally for `appimage`, so the setuid helper is never invoked (this is also why `_doctor_check_effective_sandbox` already short-circuits on that package type). `run_doctor` now forwards its package type to `_doctor_check_chrome_sandbox`, which on `appimage` emits a single informational `Chrome sandbox: not used (AppImage runs with --no-sandbox)` and returns without stat-ing anything; deb/rpm/nix behaviour is unchanged, and the not-found `[WARN]` — still meaningful where the helper is supposed to be installed — now names the path it looked at instead of claiming "expected for AppImage". Two mutation-checked bats cases: the appimage gate (worst-case 0644 adjacent sandbox must yield the info line, no `stat` call, and no FAIL/WARN/PASS) and a blast-radius guard that the same bad file still FAILs for `deb`. ([#785](https://github.com/aaddrick/claude-desktop-debian/issues/785))
...
- A release-job failure no longer withholds the build from every publish channel over a missing convenience asset: `asar` and `prettier` were installed by a step gated on the `claude-desktop-versions` checkout, which is `continue-on-error: true` — so when that repo went private the checkout rendered as a green ✓ while its `outcome` skipped the install, and the unrelated reference-source step exited 127 and failed the job. Because `mirror-official-deb`, `update-apt-repo`, `update-dnf-repo` and `update-aur-repo` all carry `needs: [release]`, `1.24012.9` published to no channel until the job was re-run by hand. The tooling is now installed unconditionally and the reference-source step degrades with a warning like its two existing bail-outs, with the invariants pinned by `tests/ci-release-job.bats`. ([#813](https://github.com/aaddrick/claude-desktop-debian/issues/813), [#814](https://github.com/aaddrick/claude-desktop-debian/pull/814))
- The release job's reference-source tooling runs on Node 22, so the `asar` it installs is the one this repo asked for rather than whichever the runner's npm picks. [#814](https://github.com/aaddrick/claude-desktop-debian/pull/814) moved that install out from behind the `claude-desktop-versions` gate but left the step feeding it on `node-version: "20"`, and `@electron/asar` has declared `engines.node >=22.12.0` since 4.0.0. The mismatch never fails the install; what it does depends on npm. A bare `npm install -g <name>` is a *range* spec (`*`), not a tag, and `npm-pick-manifest` >=9.1.0 stops short-circuiting a `*` range to the `latest` dist-tag when its `engines` mismatch — so the runner quietly installs asar 3.4.1 and `@anthropic-ai/claude-code` 2.1.197 (`>=18.0.0`) instead of 4.3.0 and 2.1.270. On older npm, which keeps the shortcut, the same command takes 4.3.0 and the `EBADENGINE` mismatch is only a **warning**, so the wrapper lands on disk and dies on first use — and there the `command -v asar` guard #814 adds does not help, because the binary *is* on `PATH`: `asar extract` fails, the step carries no `continue-on-error`, and the release job takes the four publish jobs on `needs: [release]` down with it, reaching the same outage #814 exists to prevent through a different door. Verified against the live registry: the behaviour changes at `npm-pick-manifest` 9.1.0 (8.0.2, 9.0.0 and 9.0.1 all resolve 4.3.0 under Node 20.19.2; 9.1.0 and 10.0.0 resolve 3.4.1), which npm bundles from 10.8.2 — and 10.8.2 is exactly what Node 20.19.2 ships, so `setup-node` with `"20"` lands on the silent-downgrade side by one patch release. npm 10.0.0 through 10.8.0 bundle 9.0.x and take the install-and-die side. Neither outcome announces itself, so the runtime is pinned rather than left to the runner. Same class as the build-side fix in [#841](https://github.com/aaddrick/claude-desktop-debian/pull/841), which left `ci.yml` alone to avoid conflicting with #814, and as the triage workflows in [#847](https://github.com/aaddrick/claude-desktop-debian/pull/847). Pinned by a new `tests/ci-release-job.bats` case; mutation-checked — reverting the bump, deleting the `node-version` key, and commenting it out each red it. ([#839](https://github.com/aaddrick/claude-desktop-debian/issues/839), [#845](https://github.com/aaddrick/claude-desktop-debian/pull/845))
- The issue-triage workflows run on Node 22, so the versions of the tools they install stop depending on which npm the runner ships. `issue-triage.yml` set up `node-version: "20"` at four sites and then `npm install -g`'d `@electron/asar prettier` (the `fetch-reference` job) and `@anthropic-ai/claude-code` (the three Claude CLI jobs) on top of it, but asar has declared `engines.node >=22.12.0` since 4.0.0 and the claude-code CLI declares `>=22.0.0`. Neither install fails loudly on Node 20; what it does instead depends on npm. A bare `npm install -g <name>` is a *range* spec (`*`), and `npm-pick-manifest` >=9.1.0 stops short-circuiting a `*` range to the `latest` dist-tag when its `engines` mismatch, so npm quietly resolves asar to 3.4.1 and the CLI to 2.1.197 (`>=18.0.0`), 73 releases behind what Node 22 gets. On older npm, which keeps the shortcut, the same command resolves 4.3.0 and the `EBADENGINE` mismatch is only a **warning**, so the wrapper installs and then dies on every invocation — the shape [#839](https://github.com/aaddrick/claude-desktop-debian/issues/839) was reported as, on a Debian 13 host whose npm is packaged separately from its Node. Both outcomes are silent, and which one a triage run gets was a property of the runner rather than of anything in the repo — npm bundles 9.1.0 from 10.8.2, which is precisely what Node 20.19.2 ships, so `setup-node` with `"20"` cleared the boundary by a single patch release while npm 10.0.0 through 10.8.0 fall the other way (measured against the live registry: pick-manifest 8.0.2/9.0.0/9.0.1 resolve 4.3.0 under Node 20.19.2, 9.1.0/10.0.0 resolve 3.4.1). v1 is `workflow_dispatch`-only since v2 took over production routing, so this was latent rather than firing, and `issue-triage-v2.yml` was already on 22. Pinned by `tests/workflow-node-floor.bats`, which asserts the floor file-wide across `ci.yml`, `issue-triage.yml` and `issue-triage-v2.yml` rather than at the repaired sites, so a newly added step that reintroduces 20 reds too. It absorbs the equivalent assertion that [#845](https://github.com/aaddrick/claude-desktop-debian/pull/845) had added to `tests/ci-release-job.bats`: one invariant asserted in two files drifts, and that pair had already drifted once when the npm mechanism was corrected in one copy and missed in the other. Mutation-checked — each of the six `node-version` keys across the three workflows dropped to 20, the keys deleted, the keys commented out, and a workflow renamed out from under the list each turn a test red. ([#839](https://github.com/aaddrick/claude-desktop-debian/issues/839), [#847](https://github.com/aaddrick/claude-desktop-debian/pull/847))
