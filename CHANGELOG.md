...
- The Code-tab integrated terminal now spawns a working shell on Linux instead of showing "Shell exited" and logging `Shell PTY … exited with code 1`. Two Linux-only root causes, both required: node-pty's `spawn-helper` is now compiled from source and staged next to `pty.node` — upstream `binding.gyp` gates that target to `OS=="mac"`, so neither `npm install` nor `node-gyp rebuild` emit it on Linux, yet `lib/unixTerminal.js` execs it on every unix ([#727](https://github.com/aaddrick/claude-desktop-debian/issues/727)) — and the integrated-terminal shell selector, hardcoded to `powershell.exe` with no platform branch, is repointed at `process.env.SHELL||"/bin/bash"` ([#728](https://github.com/aaddrick/claude-desktop-debian/issues/728)). Fixed for the npm build path (deb, RPM, AppImage) and the Nix build.
