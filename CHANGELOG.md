...
- The issue-triage workflows run on Node 22, so the versions of the tools they install stop depending on which npm the runner ships. `issue-triage.yml` set up `node-version: "20"` at four sites and then `npm install -g`'d `@electron/asar prettier` (the `fetch-reference` job) and `@anthropic-ai/claude-code` (the three Claude CLI jobs) on top of it, but asar has declared `engines.node >=22.12.0` since 4.0.0 and the claude-code CLI declares `>=22.0.0`. Neither install fails loudly on Node 20; what it does instead depends on npm. A bare `npm install -g <name>` is a *range* spec (`*`), and `npm-pick-manifest` >=9 — bundled by npm 10.x, which is what `setup-node` ships with Node 20 — filters ranges by `engines`, so npm quietly resolves asar to 3.4.1 and the CLI to 2.1.197 (`>=18.0.0`), 73 releases behind what Node 22 gets. On npm <=9, which does not filter, the same command resolves 4.3.0 and the `EBADENGINE` mismatch is only a **warning**, so the wrapper installs and then dies on every invocation — the shape [#839](https://github.com/aaddrick/claude-desktop-debian/issues/839) was reported as, on a Debian 13 host whose npm is packaged separately from its Node. Both outcomes are silent, and which one a triage run gets was a property of the runner rather than of anything in the repo. v1 is `workflow_dispatch`-only since v2 took over production routing, so this was latent rather than firing, and `issue-triage-v2.yml` was already on 22. Pinned by `tests/triage-node-floor.bats`, which asserts the floor across both files rather than at the four repaired sites, so a newly added step that reintroduces 20 reds too; mutation-checked — each of the five `node-version` keys dropped to 20, the keys deleted, the keys commented out, and the workflow renamed out from under the list each turn a test red. ([#839](https://github.com/aaddrick/claude-desktop-debian/issues/839), [#847](https://github.com/aaddrick/claude-desktop-debian/pull/847))
