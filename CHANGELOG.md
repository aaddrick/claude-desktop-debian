...
- The release job's reference-source tooling runs on Node 22, so the `asar` binary it installs can actually start. [#814](https://github.com/aaddrick/claude-desktop-debian/pull/814) moved that install out from behind the `claude-desktop-versions` gate but left the step feeding it on `node-version: "20"`, and `@electron/asar` has declared `engines.node >=22.12.0` since 4.0.0 — every 4.x release refuses to start below it. npm reports the mismatch as an `EBADENGINE` **warning** and installs the wrapper anyway, and an unpinned `npm install -g @electron/asar` does land on 4.x under Node 20 because a bare spec resolves through the `latest` dist-tag, which is not engine-filtered (verified against the live registry with `npm-pick-manifest`: 20.19.2, 22.11.0, 22.12.0 and 24.20.0 all resolve to 4.3.0). The `command -v asar` guard #814 added does not cover this, because the binary *is* on `PATH` and only fails when invoked — so `asar extract` would have failed the release job and skipped the four publish jobs on `needs: [release]`, reaching the same outage #814 exists to prevent through a different door. `@anthropic-ai/claude-code` declares `>=22.0.0` and was on the wrong side of the same floor; `prettier` is `>=14`. Same class as the build-side fix in [#841](https://github.com/aaddrick/claude-desktop-debian/pull/841), which left `ci.yml` alone to avoid conflicting with #814. Pinned by a new `tests/ci-release-job.bats` case; mutation-checked — reverting the bump, deleting the `node-version` key, and commenting it out each red it. ([#839](https://github.com/aaddrick/claude-desktop-debian/issues/839), [__PRNUM__](https://github.com/aaddrick/claude-desktop-debian/pull/__PRNUM__))
