...
- Closing the main window on a Linux session with no usable system tray now quits the app instead of hiding it into a tray that nothing draws — previously this stranded an invisible, `Ctrl+Q`-only background process. The wrapper's close-to-tray path was gated only on platform plus `CLAUDE_QUIT_ON_CLOSE`, never on whether a StatusNotifier host is actually registered, so on vanilla GNOME without an AppIndicator extension (the tray is "on" but nothing renders it) the window vanished into nowhere. The launcher now probes the session bus for a registered StatusNotifier host and honours the `menuBarEnabled` preference, exporting `CLAUDE_TRAY_AVAILABLE`; `frame-fix-wrapper.js` folds it into the existing close-to-tray gate rather than adding a second close handler. ([#732](https://github.com/aaddrick/claude-desktop-debian/pull/732), fixes [#66](https://github.com/aaddrick/claude-desktop-debian/issues/66), [#122](https://github.com/aaddrick/claude-desktop-debian/issues/122))
