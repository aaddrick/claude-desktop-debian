...
- `_resolve_asar` now falls through to the pinned `@electron/asar@<major>` install when an `asar` already on `PATH` fails its run-check, instead of stopping there. [#846](https://github.com/aaddrick/claude-desktop-debian/pull/846) resolved a `PATH` asar first and treated a failed run-check as terminal, which is correct diagnosis but leaves the machine broken when the install two lines below is the fix for that exact binary — a stale global 4.x left by an npm 9 that took `latest` is the [#839](https://github.com/aaddrick/claude-desktop-debian/issues/839) host (Debian 13, Node 20.19.2) plus one `npm i -g @electron/asar`. Falling through is safe precisely because it is gated on the run-check: a tool that cannot run is not a staged tool being overridden, it is a dead one, so a *working* `PATH` asar still wins outright with no install attempted. The order that matters is what happens when both fail. A naive fallback would replace the accurate `CANNOT RUN WITH NODE 20.19.2` with `Failed to install @electron/asar@3` on any offline or proxied host — pointing at the network instead of at the Node floor, which is the exact diagnostic regression #839 exists to prevent — so the `PATH` refusal is held rather than printed at the fallback point and re-emitted to *lead* the final report, below npm's own error spew where the operator is actually looking; every exit that leaves the caller without an asar still names the Node floor. The run-check itself is extracted to `_asar_probe` (both arms unchanged: exit code and stdout shape judged separately, merged streams reported, the match anchored at the start of the reply) and the install to `_asar_install`, so the resolver reaches each from two places without duplicating them. `tests/asar-resolve.bats` grew to 30 tests and its `setup()` now installs a *failing* npm stub by default, so no unit test can reach the real registry — removing that default is itself mutation-checked and shows four tests would otherwise hit `registry.npmjs.org`. Seventeen mutations each turn a different test red, including deleting the fallback, dropping the `PATH` refusal from either failure report, dropping the Node floor from either, leaving `$asar_exec` pointed at the dead `PATH` binary after a successful fallback, and dropping the `v?` from the version regex (the shipped 3.4.1 wrapper answers `v3.4.1`, so that one rejects the very version we pin). ([#849](https://github.com/aaddrick/claude-desktop-debian/issues/849))
