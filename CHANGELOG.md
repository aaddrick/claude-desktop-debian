...
- The issue-triage workflows run on Node 22, so the tools they install can actually start. `issue-triage.yml` set up `node-version: "20"` at four sites and then `npm install -g`'d `@electron/asar prettier` (the `fetch-reference` job) and `@anthropic-ai/claude-code` (the three Claude CLI jobs) on top of it, but asar has declared `engines.node >=22.12.0` since 4.0.0 and the claude-code CLI declares `>=22.0.0`. npm reports either mismatch as an `EBADENGINE` **warning**, so the install succeeds and leaves a binary that exists and never runs — the same trap as [#839](https://github.com/aaddrick/claude-desktop-debian/issues/839) on the build side and [#845](https://github.com/aaddrick/claude-desktop-debian/pull/845) in the release job. v1 is `workflow_dispatch`-only since v2 took over production routing, so this was latent rather than firing, and `issue-triage-v2.yml` was already on 22. Pinned by `tests/triage-node-floor.bats`, which asserts the floor across both files rather than at the four repaired sites, so a newly added step that reintroduces 20 reds too; mutation-checked — each of the five `node-version` keys dropped to 20, the keys deleted, the keys commented out, and the workflow renamed out from under the list each turn a test red. ([#839](https://github.com/aaddrick/claude-desktop-debian/issues/839), [#847](https://github.com/aaddrick/claude-desktop-debian/pull/847))
