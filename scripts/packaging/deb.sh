...
# as the google-chrome, code, and slack packages do. The profile is created
# here (not shipped as a dpkg file) only when AppArmor can parse the userns
# rule (4.0+); on older AppArmor or systems without it the rule is skipped,
# which is harmless since those systems don't impose the restriction.
...
# unload and delete it ourselves on package removal. Skip on upgrade — the
# incoming postinst rewrites and reloads it.
