...
- **[colonelpanic8](https://github.com/colonelpanic8)** for diagnosing that the GPU-recovery path disabled Chromium's software rasterizer alongside the GPU, turning a fallback into the fatal "GPU process isn't usable" abort, and for the fix and test coverage that keeps the rasterizer available (#738)
- **[CochainComplex](https://github.com/CochainComplex)** for diagnosing that a package upgrade underneath a running Claude Desktop leaves a stale instance holding Electron's single-instance lock with a deleted executable, so the next launch silently does nothing; for the launcher cleanup that terminates only such `(deleted)`-exe instances, its bats and artifact-level coverage, and for the patch-stage harness fix that resolves a real, runnable asar instead of falling through to `npx` (#833, #834)
- **[CochainComplex](https://github.com/CochainComplex)** for closing two long-open test/doctor gaps: the rpm artifact test now installs once with `--noscripts` so the chrome-sandbox setuid bit is proven to come from the payload rather than a scriptlet (#617, #843), and `--doctor` no longer fails the chrome-sandbox permission check on AppImage runs, where the bit is dropped by design and the sandbox is never used (#785, #844); and the SingletonLock liveness check in both `--doctor` and the launcher now requires the holder to be a Claude Desktop process by executable path, so a recycled PID no longer turns a stale lock into a false PASS (#784, #842)
- **[lizthegrey](https://github.com/lizthegrey)** for tracing why a Cowork VM bundle created before Anthropic's manifest grew a native `unix` entry keeps ~11 GB of dead `rootfs.vhdx` / `rootfs.vhdx.zst` alongside the working `rootfs.img` (the pre-3.0 Linux support repurposed the win32 VHDX entries, and nothing ever deleted them once the official `coworkd` switched formats), and for the launcher cleanup that reclaims the space, verified against the pinned 2.110.0 manifest (#855, #856)
