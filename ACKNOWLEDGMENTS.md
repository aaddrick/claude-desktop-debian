...
- **[diarized](https://github.com/diarized)**
  ...
  - The first `StartupWMClass` fix attempt and decoupling the productName tripwire from the window-class value — superseded by the build-time `desktopName` derivation once upstream renamed the identity, but the diagnosis that our `.desktop` never matched the runtime class stands (#786, #779)
...
- **[yannicklescure](https://github.com/yannicklescure)** for diagnosing that the 2.x-era `claude-desktop-bwrap` AppArmor profile survived every upgrade (the postrm cleanup never fires on the upgrade path) and collided with Ubuntu 26.04's `bwrap-userns-restrict`, breaking glycin image decoding down to the GDM greeter; for the postinst cleanup that removes it, the artifact-test pin, and the symptom-first troubleshooting entry (#542, #825); and for raising the build's Node floor to `@electron/asar`'s engine constraint, with a run-once asar probe that names the Node floor and the Node in use, so a Debian 13 host fails at the tooling stage instead of at a patch anchor three stages later, backed by a mutation-checked bats suite (#839, #841); for the same floor on the release job's Node (#845); and for the shared pinned `_resolve_asar` in `scripts/_common.sh` that replaces every `npx --yes @electron/asar` call in the audit tool and the artifact tests, turning the artifact tests' silent `[PASS]` skip on an unusable asar into a real failure (#839, #846); and for making the patch-necessity audit tool probe the whole `.vite/build` set instead of one resolved main chunk, so it runs again on every bundle upstream has shipped since 1.26832.0, fixing the per-file `grep -c` undercount, the locale-dependent chunk order, the BRE-to-PCRE pipe, the retired `apt_channel_pending` updater anchor, and the stale `--help` range along the way, all pinned by a 25-test bats suite (#850, #852); and for making `_resolve_asar` self-healing — a PATH asar that fails the run-check now falls through to the pinned `@electron/asar@3` install while the report still leads with the original refusal and names the Node floor, with the bats suite made network-proof by default (#849, #851); and for carrying the Node floor to the issue-triage workflows, with one `workflow-node-floor.bats` suite asserting every floored workflow in one place instead of a per-file copy that had already drifted once (#839, #847)
