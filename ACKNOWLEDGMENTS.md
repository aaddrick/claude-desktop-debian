...
- **[lizthegrey](https://github.com/lizthegrey)**
  ...
  - Tracing why a Cowork VM bundle created before Anthropic's manifest grew a native `unix` entry keeps ~11 GB of dead `rootfs.vhdx` / `rootfs.vhdx.zst` alongside the working `rootfs.img` (the pre-3.0 Linux support repurposed the win32 VHDX entries, and nothing ever deleted them once the official `coworkd` switched formats), and the launcher cleanup that reclaims the space, verified against the pinned 2.110.0 manifest (#855, #856)
...
- **[JustinJLeopard](https://github.com/JustinJLeopard)**
  - Detecting missing electron binaries after Node 24's `extract-zip` silently no-ops, with an `unzip` fallback that recovers from the `@electron/get` cache (#631, #584)
  - AppStream metainfo that surfaces the package in GNOME Software, KDE Discover, and App Center, wired into the deb, rpm, and AppImage builds (#633)
...
- **[diarized](https://github.com/diarized)**
  - Auto-installing scoped AppArmor userns profiles from the `.deb` postinst on Ubuntu 24.04+ — one for the bundled Electron binary (fixing the launch crash without `--no-sandbox`) and one for `/usr/bin/bwrap` (keeping Cowork's sandbox isolated instead of silently falling back to host-direct), automating the workaround from #351 (#687, #694)
  - First `StartupWMClass` fix attempt, and decoupling the productName tripwire from the window-class value — superseded by the build-time `desktopName` derivation once upstream renamed the identity, but the diagnosis that our `.desktop` never matched the runtime class stands (#786, #779)
...
- **[colonelpanic8](https://github.com/colonelpanic8)**
  - Independently reproducing the same 1.13576+ startup hang and contributing BATS coverage for the Linux native stub (#730)
  - Diagnosing that the GPU-recovery path disabled Chromium's software rasterizer alongside the GPU, turning a fallback into the fatal "GPU process isn't usable" abort, and the fix and test coverage that keeps the rasterizer available (#738)
...
- **[yannicklescure](https://github.com/yannicklescure)**
  - Diagnosing that the 2.x-era `claude-desktop-bwrap` AppArmor profile survived every upgrade (the postrm cleanup never fires on the upgrade path) and collided with Ubuntu 26.04's `bwrap-userns-restrict`, breaking glycin image decoding down to the GDM greeter; the postinst cleanup that removes it, the artifact-test pin, and the symptom-first troubleshooting entry (#542, #825)
  - Raising the build's Node floor to `@electron/asar`'s engine constraint, with a run-once asar probe that names the Node floor and the Node in use, so a Debian 13 host fails at the tooling stage instead of at a patch anchor three stages later, backed by a mutation-checked bats suite (#839, #841)
  - The same floor on the release job's Node (#845)
  - The shared pinned `_resolve_asar` in `scripts/_common.sh` that replaces every `npx --yes @electron/asar` call in the audit tool and the artifact tests, turning the artifact tests' silent `[PASS]` skip on an unusable asar into a real failure (#839, #846)
  - Making the patch-necessity audit tool probe the whole `.vite/build` set instead of one resolved main chunk, so it runs again on every bundle upstream has shipped since 1.26832.0, fixing the per-file `grep -c` undercount, the locale-dependent chunk order, the BRE-to-PCRE pipe, the retired `apt_channel_pending` updater anchor, and the stale `--help` range along the way, all pinned by a 25-test bats suite (#850, #852)
  - Making `_resolve_asar` self-healing — a PATH asar that fails the run-check now falls through to the pinned `@electron/asar@3` install while the report still leads with the original refusal and names the Node floor, with the bats suite made network-proof by default (#849, #851)
  - Carrying the Node floor to the issue-triage workflows, with one `workflow-node-floor.bats` suite asserting every floored workflow in one place instead of a per-file copy that had already drifted once (#839, #847)
  - Closing the hole between the two Node-floor tests: a `setup-node` step with no `node-version` at all took the runner's default unnoticed, so the suite now asserts step count equals key count per workflow, reproduced first as a single-key deletion the original all-keys mutation never tried (#847, #866)
  - Pinning the `@anthropic-ai/claude-code` CLI the release job installs, the last floating install in the repo, with a release-job test that counts its matches before judging them so a comment naming the package cannot pass it vacuously (#869)
  - Finding that the v1 triage fallback never received v2's hardening (unpinned CLI, `--dangerously-skip-permissions` over issue text, persisted credentials) and retiring it, with a file-wide test that now holds v2 to its own posture; along the way, that `/triage` had been dispatching a workflow name that no longer existed (#867, #868)
  - Tracing the triage classifier's phantom labels to its own prompt, which listed four names the repo never had, and replacing the hardcoded list with the live `gh label list` set interpolated into the prompt, plus the `security` criterion and a test that intersects every name the pipeline still states by hand against the real label set (#867, #870)
  - Pinning the launcher's eight pre-launch hygiene calls in all three packaging scripts with a heredoc-aware structural suite — presence, order against `setup_logging` and the exec, containment, and the per-format `heal_autostart_entry` argument — so a call dropped from one launcher can no longer ship green (#857, #872)
  - Making the artifact launch smoke test ask the X server for a mapped window carrying the artifact's own `WM_CLASS`, so a main process that survives with no UI at all no longer passes — the harness now owns its `Xvfb` via `-displayfd`, reads the class off the launcher's own `Executing:` line, reaps the server on every path, and states plainly what a mapped window does and doesn't prove; the `show:`/`ready-to-show` premise was checked against the pinned bundle rather than assumed (#616, #873)
  - Definition-level coverage for `backup_user_config`, the config-wipe recovery rotation that had no test at any level: byte-identical copies, the pre-wipe copy surviving at `.2`, no rotation on an unchanged file, the five-slot cap, Cowork store flattening, the XDG fallback, and fail-safe on an unwritable dir, with a function-scoped mutation pass that also caught its own decorative assertion (#857, #874)
  - Hardening the tray-icon patch's callee capture after two releases shipped a silently broken tray: a symmetric property-chain capture, a splice-point allowlist that a denylist would have got wrong (`await x()`, `this.#x()`), an idempotency check the corrupted bytes cannot satisfy, and eight fixtures from real shipped bundles, including the pre-fix corruption reproduced byte-for-byte (#820, #875), and documenting the one path where `CLAUDE_TRAY_USE_DARK_ICON=0` yields to upstream's tray-rebuild retry (#876, #878)
  - Shipping an apport crash blacklist in the `.deb` as a registered conffile, closing the journald-flood half of #582: the Electron ELF and the crashpad handler keyed to `$package_name`, a source-level suite plus real-package assertions, and a troubleshooting entry for hosts on an older build (#582, #880)
  - Pinning the quit-time reap of the opt-in bwrap Cowork daemon with real-process legs, a SIGTERM reap and a SIGTERM-ignoring daemon forced through the SIGKILL escalation, scoped so a bystander naming the script is never signalled, plus a structural pin on the session spawn's `--unshare-pid --die-with-parent --new-session` flags (#369, #881)
- **[CochainComplex](https://github.com/CochainComplex)**
  - Diagnosing that a package upgrade underneath a running Claude Desktop leaves a stale instance holding Electron's single-instance lock with a deleted executable, so the next launch silently does nothing; the launcher cleanup that terminates only such `(deleted)`-exe instances, its bats and artifact-level coverage, and the patch-stage harness fix that resolves a real, runnable asar instead of falling through to `npx` (#833, #834)
  - Closing three long-open test/doctor gaps: the rpm artifact test now installs once with `--noscripts` so the chrome-sandbox setuid bit is proven to come from the payload rather than a scriptlet (#617, #843), and `--doctor` no longer fails the chrome-sandbox permission check on AppImage runs, where the bit is dropped by design and the sandbox is never used (#785, #844); and the SingletonLock liveness check in both `--doctor` and the launcher now requires the holder to be a Claude Desktop process by executable path, so a recycled PID no longer turns a stale lock into a false PASS (#784, #842)
- **[cbnuke](https://github.com/cbnuke)** for reporting the 190 GB `/var/log/syslog` flood on Ubuntu with the `/var/crash` evidence that pinned it to an apport feedback loop, and confirming in the field that blacklisting the binary stopped the growth (#582, #880)
- **[kkkzbh](https://github.com/kkkzbh)** for making `CLAUDE_FORCE_SANDBOX` work from the persistent launcher config, so the Wayland sandbox override applies to app-menu launches too, with config-path regression tests across deb/RPM and Nix (#804, #886)
