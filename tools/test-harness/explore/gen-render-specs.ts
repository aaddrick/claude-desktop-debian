...
function generateSpec(
	...
	out.push(
		...
		'// userLoaded level). With CLAUDE_TEST_USE_HOST_CONFIG=1 the harness',
		"// kills any running host Claude, copies the auth-relevant subset of",
		"// ~/.config/Claude into a hermetic tmpdir, and launches against",
		"// that copy — host config is left untouched after the kill+seed.",
		'//',
		...
		'',
		"// Single shared launch + inspector across the whole suite. 383",
		...
		'\t\tif (!useHostConfig) {',
		"\t\t\tsuiteSkipReason = 'CLAUDE_TEST_USE_HOST_CONFIG=1 not set — render ' +",
		"\t\t\t\t'sweep needs auth lifted from the host config to reach the ' +",
		"\t\t\t\t'surfaces the inventory was walked from.';",
		'\t\t\treturn;',
		'\t\t}',
